...
public class NotificationController : Controller
{
    ...
    private void RevealContactInfo(int patientId, int donorId)
    {
        ...
        if (patient != null && donor != null)
        {
            ...
            _notificationService.SendNotification($"Donor contact info: {donor.ContactInfo}", patientId);
            _notificationService.SendNotification($"Patient contact info: {patient.ContactInfo}", donorId);
        }
    }
    ...
    private void BlockDonor(int donorId, int days)
    {
        ...
        if (donor != null)
        {
            donor.BlockUntil = DateTime.Now.AddDays(days);
            _userRepository.UpdateUser(donor);
        }
    }
}
